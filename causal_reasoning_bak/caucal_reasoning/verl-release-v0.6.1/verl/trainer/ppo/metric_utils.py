...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Extra agent-loop metrics: aggregate numeric non-tensor fields (e.g., backward/* step reward stats).
    # These are emitted by some agent loops via extra_fields["reward_extra_info"] and land in non_tensor_batch.
    for key, arr in batch.non_tensor_batch.items():
        if not isinstance(key, str) or not key.startswith("backward/"):
            continue
        if not isinstance(arr, np.ndarray):
            continue
        # Skip object arrays (likely lists/strings).
        ...
        if a.size == 0:
            continue
        metrics[f"{key}/mean"] = float(a.mean())
        metrics[f"{key}/max"] = float(a.max())
        metrics[f"{key}/min"] = float(a.min())
