...
class AdvantageEstimator(str, Enum):
    ...
    GAE = "gae"
    GRPO = "grpo"
    GRPO_TOKEN_LEVEL = "grpo_token_level"  # Token-level GRPO using step rewards directly
...
@register_adv_est(AdvantageEstimator.GRPO_TOKEN_LEVEL)
def compute_grpo_token_level_advantage(
    ...
    """
    Token-level GRPO: Use per-token rewards directly from agent loop (step rewards + final reward).
    
    Instead of summing all token rewards and computing a scalar advantage, this method:
    1. Uses rm_scores (step_reward per token + final_reward/num_tokens) directly
    2. Computes per-token advantage by normalizing within each group
    
    This allows step-level credit assignment to be preserved in the GRPO update.
    
    Args:
        token_level_rewards: (bs, response_length) - fallback rewards if rm_scores not provided
        response_mask: (bs, response_length) - mask for valid tokens
        index: grouping index for GRPO normalization
        epsilon: small value for numerical stability
        norm_adv_by_std_in_grpo: whether to normalize by std
        config: algorithm config
        rm_scores: (bs, response_length) - per-token rewards from agent loop (step + final)
