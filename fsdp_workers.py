           ...
           # ) 
            ...
            for name, param in params.items():
                if isinstance(param, DTensor):
                    ...
                    # 2. 准备容器 (假设 FSDP 展平后在 dim=0 切分)
                    gathered_tensors = [torch.zeros_like(local_tensor) for _ in range(dist.get_world_size())]
                    # 3. 手动执行最基础的 all_gather (这个通常不会报错)
                    dist.all_gather(gathered_tensors, local_tensor)
                    # 4. 拼起来并移到目标设备
                    full_param = torch.cat(gathered_tensors, dim=0).to(device)
                    per_tensor_param.append((name, full_param))
                else:
                    per_tensor_param.append((name, param))
