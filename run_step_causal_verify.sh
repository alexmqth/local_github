...
#	  echo "[FATAL] nvcc not found in PATH. Please install cuda-nvcc/cuda-toolkit in this env or load CUDA module."
#	    exit 1
#fi
...
# strict check (optional but recommended)
if [ ! -x "$CUDACXX" ]; then
...
export LDFLAGS="-L$CONDA_PREFIX/lib -L$CONDA_PREFIX/lib64 ${LDFLAGS:-}"
...
# some libs look for NVCC explicitly
export NVCC="$CUDACXX"
...
# ---- CUDA toolkit env vars (use conda nvcc path) ----
#export CUDA_HOME=$(dirname "$(dirname "$(which nvcc)")")
#export PATH="$CUDA_HOME/bin:$PATH"
# conda cudatoolkit libraries are usually here
#export LD_LIBRARY_PATH="$CUDA_HOME/lib64:$CUDA_HOME/lib:$LD_LIBRARY_PATH"
# explicitly tell build tools which nvcc to use
...
TRAIN_PARQUET=${TRAIN_PARQUET:-/home/projects/hku-medai/larrypl/code/mq/causal-reasoning/verl/verl/data/math/causal_output/pns_agentloop_gsm8k.parquet}
