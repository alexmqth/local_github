...
class StepCausalVerifierAgentLoop(StepVerifyAgentLoop):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        while True:
            ...
            # If a final answer marker is present, stop asking for more steps.
            if finished:
                # Record this step with current PNS score and terminate.
                if str(cfg.pns_mode).lower() == "discrete":
                    passed = bool(float(pns) >= 0.5)
                    step_reward = float(cfg.pass_reward) if passed else float(cfg.fail_reward)
                else:
