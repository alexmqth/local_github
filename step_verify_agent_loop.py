...
_LATEX_FRAC_RE = re.compile(r"\\(?:frac|dfrac|tfrac)\s*\{\s*([-+]?\d+)\s*\}\s*\{\s*([-+]?\d+)\s*\}")
_SLASH_FRAC_RE = re.compile(r"([-+]?\d+)\s*/\s*([-+]?\d+)")
_LATEX_WRAP_PAREN_RE = re.compile(r"^\s*\\\((.*)\\\)\s*$", flags=re.DOTALL)
_LATEX_WRAP_BRACK_RE = re.compile(r"^\s*\\\[(.*)\\\]\s*$", flags=re.DOTALL)
_LATEX_WRAP_DOLLAR_RE = re.compile(r"^\s*\$(.*)\$\s*$", flags=re.DOTALL)


def _strip_latex_wrappers(s: str) -> str:
    for pat in (_LATEX_WRAP_PAREN_RE, _LATEX_WRAP_BRACK_RE, _LATEX_WRAP_DOLLAR_RE):
        m = pat.match(s)
        if m:
...
def _looks_like_complex_latex(s: str) -> bool:
    t = s
    if "\\" in t:
        if any(k in t for k in ("\\begin", "\\end", "\\sqrt", "\\frac", "\\dfrac", "\\tfrac", "\\pmatrix")):
            return True
...
def _normalize_answer_str(x: str) -> str:
    ...
    # Try LaTeX fractions first: \frac{a}{b}
    ...
    # Try simple fractions: a/b
    m = _SLASH_FRAC_RE.search(s)
    if m:
        try:
            num = int(m.group(1))
            den = int(m.group(2))
    ...
    # Try decimal/integer: normalize to reduced fraction to compare with fraction answers
    m = _NUM_RE.search(s)
    if m:
        try:
            f = Fraction(m.group(0))
            return str(f.numerator) if f.denominator == 1 else f"{f.numerator}/{f.denominator}"
        except Exception:
            return m.group(0)

    # Normalize LaTeX expressions for non-numeric answers (e.g., matrices)
    if _looks_like_complex_latex(s):
        return _normalize_latex_expr(s).lower()
...
def _has_answer_marker(text: str) -> bool:
    if not isinstance(text, str):
        return False
    t = text
    # Accept common final formats (kept strict but include boxed)
    if _ANSWER_TAG_RE.search(t):
        return True
    if _GSM8K_RE.search(t):
        return True
    if re.search(r"\\boxed\s*\{", t):
        return True
    if re.search(r"\bfinal\s+answer\b", t, flags=re.IGNORECASE):
        return True
    if re.search(r"\banswer\s+is\b", t, flags=re.IGNORECASE):
        return True
    if re.search(r"^\s*ANSWER\s*:", t, flags=re.IGNORECASE | re.MULTILINE):
        return True
    return False
...
def _extract_final_answer(text: str) -> Optional[str]:
    ...
    if m:
        ...
        # Handle incomplete tag: "<ANSWER>37..." without closing
        m_open = re.search(r"<\s*answer\s*>\s*([^\n\r<]+)", t, flags=re.IGNORECASE)
        if m_open:
            t2 = m_open.group(1).strip()
        else:
            t2 = t
...
class StepVerifyAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        cls.verifier_cfg = _VerifierConfig(
            ...
            min_assistant_turns_before_answer=int(step_cfg.get("min_assistant_turns_before_answer", 0)),
            early_answer_penalty=float(step_cfg.get("early_answer_penalty", 0.0)),
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        while True:
            ...
            answer_marker_hit = _has_answer_marker(assistant_text)
            if first_answer_marker_turn is None and answer_marker_hit:
            ...
            # If a final answer marker appears, end the conversation here
            if answer_marker_hit:
                break
