...
- Stop when the policy emits an answer marker (<ANSWER>...</ANSWER> or \\boxed{...} or "final answer" or ####).
- Supports both MATH-500 (\\boxed{} format) and GSM8K (#### format) datasets.

Supported datasets:
- HuggingFaceH4/MATH-500: Uses 'problem'/'solution' fields, \\boxed{...} answer format
- openai/gsm8k: Uses 'question'/'answer' fields, #### answer format
...
def _extract_gsm8k_answer(answer_text: str) -> Optional[str]:
    ...
    Example: "Natalia sold 48/2 = <<48/2=24>>24 clips in May...#### 72"
    """
    if not isinstance(answer_text, str) or not answer_text.strip():
        return None
    # Look for #### followed by the final answer
    match = re.search(r"####\s*(.+?)$", answer_text.strip(), flags=re.MULTILINE)
    if match:
        ans = match.group(1).strip()
        # Clean up: remove commas from numbers, normalize
        ans = ans.replace(",", "")
...
def _extract_answer_gsm8k_style(text: str) -> Optional[str]:
    """
    Extract answer from model output for GSM8K evaluation.
    ...
    # Try "the answer is X" pattern (common in GSM8K outputs)
...
def _has_answer_marker(text: str) -> bool:
    ...
    if re.search(r"final\s+answer", text, flags=re.IGNORECASE):
        return True
    # GSM8K style: #### followed by answer
    if "####" in text:
        return True
    # Common pattern: "the answer is X"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Dataset type for field/answer parsing: 'math' (problem/solution/boxed), 'gsm8k' (question/answer/####), or 'auto' to detect",
    )
    ...
    if dataset_type == "auto":
        # Auto-detect based on dataset name or columns
        ...
        if "gsm8k" in ds_name_lower or "gsm" in ds_name_lower:
            ...
            if "question" in cols and "answer" in cols:
                ...
                dataset_type = "math"
        print(f"[INFO] Auto-detected dataset_type={dataset_type}")

    n_total = 0
    n_correct = 0
    desc_str = f"{args.dataset_name} inference"
    with open(args.out_jsonl, "w", encoding="utf-8") as f:
        for i, ex in enumerate(tqdm(ds, desc=desc_str)):
            # Extract problem and ground truth based on dataset type
